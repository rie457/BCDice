...
class FutariSousa < DiceBot
  ...
  def prefixs
    ...
    ['DT', '1DT', '3DT', 'AS', '1AS', '3AS', 'SHRD', 'SHFM', 'SHBT', 'SHPI', 'SHEG', 'SHWP', 'SHDS', 'SHFT', 'SHIN', 'SHEM', 'EVS', 'EVW', 'EVN', 'EVC', 'EVV', 'OBT', 'ACT', 'EWT', 'WMT', 'BGDD', 'BGDG', 'BGDM', 'BGAJ', 'BGAP', 'BGAI', 'HT', 'BT', 'GRT', 'MIT', 'JBT66', 'JBT10', 'FST66', 'FST10', 'FLT66', 'FLT10', 'LDT66', 'LDT10', 'NCT66', 'NCT10',]
  end
...
MESSAGETEXT
  ...
  def rollDiceCommand(command)
    ...
    when 'DT'
      type = 'DT'
      output,diceText = futarisousa_dt
    when '1DT'
      type = '1DT'
      output,diceText = futarisousa_1dt
    when '3DT'
      type = '3DT'
      output,diceText = futarisousa_3dt
    when 'AS'
      type = 'AS'
      output,diceText = futarisousa_as
    when '1AS'
      type = '1AS'
      output,diceText = futarisousa_1as
    when '3AS'
      type = '3AS'
      output,diceText = futarisousa_3as
    when 'SHRD'
      type = '異常な癖決定表'
      output,diceText = futarisousa_strange_habit_random
    when 'SHFM'
      type = '異常な癖・口から出る表'
      output,diceText = futarisousa_strange_habit_from_mouth
    when 'SHBT'
      type = '異常な癖・強引な捜査表'
      output,diceText = futarisousa_strange_habit_bull_through
    when 'SHPI'
      type = '異常な癖・すっとぼけ表'
      output,diceText = futarisousa_strange_habit_play_innocent
    when 'SHEG'
      type = '異常な癖・事件に夢中表'
      output,diceText = futarisousa_strange_habit_engrossed
    when 'SHWP'
      type = '異常な癖・パートナーと……表'
      output,diceText = futarisousa_strange_habit_with_partner
    when 'SHDS'
      type = '異常な癖・何かしている表'
      output,diceText = futarisousa_strange_habit_do_something
    when 'SHFT'
      type = '異常な癖・奇想天外表'
      output,diceText = futarisousa_strange_habit_fantastic
    when 'SHIN'
      type = '異常な癖・急なひらめき表'
      output,diceText = futarisousa_strange_habit_inspiration
    when 'SHEM'
      type = '異常な癖・喜怒哀楽表'
      output,diceText = futarisousa_strange_habit_emotion
    when 'EVS'
      type = '現場にて／イベント表'
      output,diceText = futarisousa_event_scene
    when 'EVW'
      type = 'なぜ？／イベント表'
      output,diceText = futarisousa_event_why
    when 'EVN'
      type = '協力者と共に／イベント表'
      output,diceText = futarisousa_event_npc
    when 'EVC'
      type = '向こうから／イベント表'
      output,diceText = futarisousa_event_coming
    when 'EVV'
      type = 'VS容疑者／イベント表'
      output,diceText = futarisousa_event_vs
    when 'OBT'
      type = '調査の障害表'
      output,diceText = futarisousa_obstruction_table
    when 'ACT'
      type = '変調表'
      output,diceText = futarisousa_abnormal_condition
    when 'EWT'
      type = '目撃者表'
      output,diceText = futarisousa_eyewitness_table
    when 'WMT'
      type = '迷宮入り表'
      output,diceText = futarisousa_wrapped_in_mystery_table
    when 'BGDD'
      type = '探偵・運命の血統 背景表'
      output,diceText = futarisousa_background_detective_destiny
    when 'BGDG'
      type = '探偵・天性の才能 背景表'
      output,diceText = futarisousa_background_detective_genius
    when 'BGDM'
      type = '探偵・マニア 背景表'
      output,diceText = futarisousa_background_detective_mania
    when 'BGAJ'
      type = '助手・正義の人 背景表'
      output,diceText = futarisousa_background_assistant_justice
    when 'BGAP'
      type = '助手・情熱の人 背景表'
      output,diceText = futarisousa_background_assistant_passion
    when 'BGAI'
      type = '助手・巻き込まれの人 背景表'
      output,diceText = futarisousa_background_assistant_involved
    when 'HT'
      type = '身長表'
      output,diceText = futarisousa_height_table
    when 'BT'
      type = 'たまり場表'
      output,diceText = futarisousa_base_table
    when 'GRT'
      type = '関係表'
      output,diceText = futarisousa_guest_relation_table
    when 'MIT'
      type = '思い出の品決定表'
      output,diceText = futarisousa_memorial_item_table
    when 'JBT66'
      type = '職業表A'
      output,diceText = futarisousa_job_table_66
    when 'JBT10'
      type = '職業表B'
      output,diceText = futarisousa_job_table_10
    when 'FST66'
      type = 'ファッション特徴表A'
      output,diceText = futarisousa_fashion_table_66
    when 'FST10'
      type = 'ファッション特徴表B'
      output,diceText = futarisousa_fashion_table_10
    when 'FLT66'
      type = '感情表A'
      output,diceText = futarisousa_feeling_table_66
    when 'FLT10'
      type = '感情表B'
      output,diceText = futarisousa_feeling_table_10
    when 'LDT66'
      type = '好きなもの／嫌いなもの表A'
      output,diceText = futarisousa_like_dislike_table_66
    when 'LDT10'
      type = '好きなもの／嫌いなもの表B'
      output,diceText = futarisousa_like_dislike_table_10
    when 'NCT66'
      type = '呼び名表A'
      output,diceText = futarisousa_name_to_call_table_66
    when 'NCT10'
      type = '呼び名表B'
      output,diceText = futarisousa_name_to_call_table_10
    end
  ...
  def futarisousa_3dt
    ...
    if(max == 1)
      "ファンブル（変調を受け、助手の心労が1点上昇）"
    elsif((dice1 == 6)or(dice2 == 6)or(dice3 == 6))
      "スペシャル（助手の余裕を1点獲得）"
    elsif(max >= 4)
      "成功"
    else
...
#異常な癖決定表
  def futarisousa_strange_habit_random
    table = [
  ...
  def futarisousa_strange_habit_from_mouth
    table = [
               [1, '猛烈に感謝の言葉を述べる'],
               [2, '皮肉ばかり言ってしまう'],
               [3, '相手の言葉を肯定してから否定する'],
  ...
  def futarisousa_strange_habit_bull_through
    table = [
  ...
  def futarisousa_strange_habit_play_innocent
    table = [
               [1, '自分の身分を偽って関係者に話を聞く'],
               [2, '情報を隠しながら話を聞く'],
  ...
  def futarisousa_strange_habit_engrossed
    table = [
  ...
  def futarisousa_strange_habit_with_partner
    table = [
               [1, 'パートナーの信頼に甘える'],
               [2, 'パートナーを置いて先に行ってしまう'],
  ...
  def futarisousa_strange_habit_do_something
    table = [
               [1, '大量の本を読んでいる'],
               [2, '好きな音楽を大音量で流している'],
               [3, '何かの数式を解いている'],
  ...
  def futarisousa_strange_habit_fantastic
    table = [
               [1, 'やめろと言われていることをする'],
               [2, 'しばらく姿を消していたが戻って来た'],
  ...
  def futarisousa_strange_habit_inspiration
    table = [
               [1, '食事をとっていたら急に謎が解ける'],
  ...
  def futarisousa_strange_habit_emotion
    table = [
               [1, '急に泣く'],
               [2, '急に怒る'],
               [3, '急に笑いだす'],
...
#調査の障害表
  def futarisousa_obstruction_table
    table = [
               [11, '探偵と助手が警察にマークされる'],
               [12, '探偵の気まぐれ'],
               [13, '探偵のやる気'],
...
#変調表
  def futarisousa_abnormal_condition
    table = [
               'すれ違い',
               '探偵の暴走',
               '喧嘩',
...
#目撃者表
  def futarisousa_eyewitness_table
    table = [
               "遊び相手が欲しい若者。判定技能：≪流行≫",
               "物覚えが悪い人。判定技能：なし",
               "忙しい人。判定技能：≪ビジネス≫",
...
#迷宮入り表  
  def futarisousa_wrapped_in_mystery_table
    table = [
...
#背景表
  def futarisousa_background_detective_destiny
  table = [
  ...
  def futarisousa_background_detective_genius
  table = [
  ...
  def futarisousa_background_detective_mania
  table = [
  ...
  def futarisousa_background_assistant_justice
    table = [
  ...
  def futarisousa_background_assistant_passion
    table = [
  ...
  def futarisousa_background_assistant_involved
    table = [
...
#関係表
  def futarisousa_guest_relation_table
    table = [
               [11, '昔のバディ'],
               [12, '友人'],
               [13, '親友'],
...
#思い出の品決定表
  def futarisousa_memorial_item_table
    table = [
...
#ファッション特徴表A
  def futarisousa_fashion_table_66
    table = [
               [11, '高級志向'],
               [12, 'スーツ'],
               [13, 'カジュアルウェア'],
...
#ファッション特徴表B
  def futarisousa_fashion_table_10
    table = [
               [1, 'スーツ'],
               [2, 'インバネスコート'],
               [3, '白衣'],
...
#好きなもの／嫌いなもの表A
  def futarisousa_like_dislike_table_66
    table = [
               [11, '死体'],
               [12, '犬'],
               [13, '猫'],
...
#好きなもの／嫌いなもの表B
  def futarisousa_like_dislike_table_10
    table = [
               [1, '犯罪'],
               [2, '謎'],
               [3, '探偵'],
...
#感情表A
  def futarisousa_feeling_table_66
    table = [
               [11, '顔'],
               [12, '雰囲気'],
               [13, '外見'],
...
#感情表B
  def futarisousa_feeling_table_10
    table = [
               [1, '人間性'],
               [2, '趣味'],
               [3, '技能'],
...
#呼び名表A
  def futarisousa_name_to_call_table_66
    table = [
               [11, 'ダーリン、ハニー'],
               [12, '呼び捨て'],
               [13, '～くん'],
...
#呼び名表B
  def futarisousa_name_to_call_table_10
    table = [
               [1, '呼び捨て'],
               [2, '～くん'],
               [3, '～さん'],
